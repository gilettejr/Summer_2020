...
class initial_processes:
    ...
    def make_tan_coord(self):
        ...
        def create_tangent_coords(galaxy_data, tangentra, tangentdec):
            ...
            xi = (np.cos(dec)*np.sin(ra-tanra))/(np.sin(dec) *
                                                 np.sin(tandec) + np.cos(dec)*np.cos(tandec)*np.cos(ra-tanra))

            # conversion for eta carried out

            eta = (np.sin(dec)*np.cos(tandec)-np.cos(dec)*np.sin(tandec)*np.cos(ra-tanra)) / \
                (np.sin(dec)*np.cos(tandec)+np.cos(dec)
                 * np.sin(tandec)*np.cos(ra-tanra))
...
# function to cut Datagalaxy_data based on cls index
    ...
    def make_deg_coord(self):
        ...
        coords = coordtransform(galaxy_data.rah, galaxy_data.ram,
                                galaxy_data.ras, galaxy_data.dd, galaxy_data.dm, galaxy_data.ds)
